...
export default function Home() {
  ...
  useEffect(() => {
    if (view.step === "expand" && view.mode === "human") {
      fetch("/api/rp-signature", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "verify-agent-v6" }) })
              ...
              <div className="space-y-3">
                ...
                {rpReady ? (
                  <>
                    ...
                    {agent && (
                      <IDKitRequestWidget open={open} onOpenChange={setOpen} app_id={appId} action="verify-agent-v6"
